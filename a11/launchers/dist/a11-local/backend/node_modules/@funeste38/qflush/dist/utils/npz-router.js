...
export function getPreferredLane(host) {
    const s = readStore();
    const entry = s[host];
    if (!entry)
        return null;
...
export function setPreferredLane(host, laneId) {
    const s = readStore();
    s[host] = { laneId, ts: Date.now() };
    writeStore(s);
}
export function lanesForHost(host, lanes = DEFAULT_LANES) {
...
export function recordFailure(host, laneId, latencyMs) {
    ...
    try {
        ...
        logger.warn(`npz-router: laneFailure.inc failed: ${err}`);
    }
    try {
        engine.scoreLane(laneId, 1, latencyMs);
    }
    catch (err) {
        logger.warn(`npz-router: engine.scoreLane failed (failure): ${err}`);
    }
}
export function recordSuccess(host, laneId, latencyMs) {
    const m = getCircuitMapForHost(host);
    m.delete(laneId);
    try {
        ...
        logger.warn(`npz-router: laneSuccess.inc failed: ${err}`);
    }
    try {
        engine.scoreLane(laneId, -1, latencyMs);
    }
    catch (err) {
        logger.warn(`npz-router: engine.scoreLane failed (success): ${err}`);
    }
}
export function isLaneTripped(host, laneId) {
...
export async function npzRoute(req, lanes = DEFAULT_LANES) {
    try {
...
export function getCircuitState(host) {
    const m = circuit.get(host);
    if (!m)
        return {};
    const out = {};
