...
export default async function runApply(argv = []) {
    try {
        ...
        try {
            ...
            logger.warn('[apply] ensure lastPath write failed: ' + String(e));
        }
        try {
            ...
            logger.warn('[apply] ensure cachePath write failed: ' + String(e));
        }
            ...
            try {
                ...
                if (approveAll) {
                    try {
                        ...
                        logger.warn('[apply] inject approve failed: ' + String(e));
                    }
                }
                if (approveId && pkt.id === approveId) {
                    try {
                        ...
                        logger.warn('[apply] inject approveId failed: ' + String(e));
                    }
                }
                ...
                try {
                    if (entry.src && fs.existsSync(entry.src)) {
                        ...
                        try {
                            ...
                            try {
                                ...
                                logger.warn('[apply] copy entry.src failed: ' + String(e2));
                            }
                            try {
                                fs.unlinkSync(entry.src);
                            }
                            catch (e) {
                                logger.warn('[apply] unlink original failed: ' + String(e));
                            }
                        }
                        // write metadata
                        try {
                            ...
                            logger.warn('[apply] write applied metadata failed: ' + String(e));
                        }
                    }
                    else {
                    ...
                    logger.warn('[apply] archive step failed: ' + String(e));
                }
            }
            catch (e) {
                ...
                try {
                    ...
                    logger.warn('[apply] move to failed folder encountered an error');
                }
            }
        }
        ...
        try {
            ...
            try {
                ...
                logger.warn('[apply] cleanup processed archives failed: ' + String(e));
            }
            try {
                archiveUtils.cleanupDatedArchives(path.join(qflushDir, 'failed'), keep);
            }
            catch (e) {
                logger.warn('[apply] cleanup failed archives failed: ' + String(e));
            }
        }
        catch (e) { }
