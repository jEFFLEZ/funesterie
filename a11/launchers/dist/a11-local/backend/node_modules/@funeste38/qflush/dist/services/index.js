...
export const GlobalState = { sleep: false };
...
function ensureQflushDir() {
    ...
    try {
        ...
        logger.warn(`[services] ensureQflushDir failed: ${String(e)}`);
    }
    return dir;
}
...
export async function jokerWipe() {
    ...
    try {
        ...
        logger.warn(`[services] attempt to kill child processes failed: ${String(e)}`);
    }
    ...
    try {
        ...
        logger.warn(`[JOKER] cleanup fs failed: ${e}`);
    }
    // persist mode
    ...
    setTimeout(() => {
        ...
        try {
            ...
            logger.warn('[services] process.exit failed: ' + String(err));
        }
    }, 300);
}
