...
export async function emitEngineState(state) {
    if (!cfg.enabled)
        return;
    ...
    if (cfg.transports.includes('file'))
        writeFileEvent(ev);
    try {
        saveTelemetryEvent('engine-' + Date.now(), 'engine_state', Date.now(), state);
    }
    catch (e) { }
    emitter.emit('telemetry', ev);
}
export async function emitRuleEvent(ev) {
    if (!cfg.enabled)
        return;
    ...
    try {
        saveTelemetryEvent('rule-' + Date.now(), 'rule_event', Date.now(), ev);
    }
    catch (e) { }
    emitter.emit('telemetry', event);
}
export async function emitDiagnostic(diag) {
    if (!cfg.enabled)
        return;
    ...
    if (cfg.transports.includes('file'))
        writeFileEvent(event);
    try {
        saveTelemetryEvent('diag-' + Date.now(), 'diagnostic', Date.now(), diag);
    }
    catch (e) { }
    emitter.emit('telemetry', event);
}
export function onTelemetry(cb) { emitter.on('telemetry', cb); }
export function shutdownCopilotBridge() { }
