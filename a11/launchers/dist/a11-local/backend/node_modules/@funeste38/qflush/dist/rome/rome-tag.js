...
export function normalizeRomePath(p) {
    let normalized = p.replace(/\\/g, '/');
    normalized = normalized.replace(/^\.?\//, '');
    normalized = normalized.replace(/\/+/g, '/');
    return normalized;
}
export function extractExt(p) {
    const ext = path.extname(p);
    return ext ? ext.slice(1) : undefined;
}
export function fnv1a24(str) {
    let hash = 0x811c9dc5;
...
export function getOrCreateRomeTag(index, meta, date = new Date()) {
    const pathNorm = normalizeRomePath(meta.path);
...
export function buildRomeTagComment(tag) {
    return `${ROME_TAG_COMMENT_PREFIX} ${toRomeTagHex(tag)}`;
}
export function parseRomeTagComment(line) {
