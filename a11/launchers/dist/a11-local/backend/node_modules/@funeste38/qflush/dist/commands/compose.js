...
export async function runCompose(argv) {
    const sub = argv[0];
    ...
    if (sub === 'up') {
        ...
        for (const m of modules) {
            ...
            if (bg) {
                ...
                await runStart({ services: [m], modulePaths: { [m]: def.path }, flags: {} });
            }
        }
        return;
    }
    ...
    if (sub === 'restart') {
        ...
        if (!def) {
            logger.info('Unknown module');
            return;
        }
        await runStart({ services: [name], modulePaths: { [name]: def.path }, flags: {} });
        return;
    }
    if (sub === 'logs') {
        const name = argv[1];
        if (!name) {
            logger.info('Specify module name');
            return;
        }
