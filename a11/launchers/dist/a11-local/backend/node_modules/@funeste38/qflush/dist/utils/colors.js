...
export function lighten(hex, percent = 0.1) {
    const { r, g, b } = hexToRgb(hex);
...
export function darken(hex, percent = 0.1) {
    const { r, g, b } = hexToRgb(hex);
...
export function readableTextColor(backgroundHex, prefer = NEZLEPHANT.whiteHex) {
...
export function styledLog(title, msg, opts) {
    let accent = NEZLEPHANT.baseHex;
    if (opts?.accent === 'joker')
        accent = NEZLEPHANT.jokerHex;
    else if (opts?.accent === 'neutral')
        accent = NEZLEPHANT.neutralHex;
