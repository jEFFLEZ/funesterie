...
process.on('unhandledRejection', (reason) => {
    try {
        console.warn('Unhandled Rejection:', reason && reason.stack ? reason.stack : String(reason));
    }
    catch (err) {
...
if (_argv.includes('--about') || _argv.includes('-a')) {
    try {
        ...
        console.log('qflush - Funesterie orchestrator');
    }
    process.exit(0);
}
...
if (typeof require !== 'undefined' && require.main === module) {
    ...
    if (first === 'apply') {
        (async () => {
            try {
                const m = await import('./commands/apply.js');
    ...
    if (first === 'resonnance') {
        (async () => {
            try {
                const m = await import('./commands/resonnance.js');
                if (!m || typeof m.default !== 'function')
                    throw new Error('resonnance command not available');
                await m.default();
                process.exit(0);
            }
            catch (err) {
