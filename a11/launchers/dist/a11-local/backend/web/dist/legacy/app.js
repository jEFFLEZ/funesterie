...
(function () {
    ...
    let pendingFiles = [];
    if ($attach) $attach.addEventListener("click", () => { if ($fileInput) $fileInput.click(); });
    ...
    // ===== OCR (Image → Texte) =====
    if ($ocrBtn) $ocrBtn.addEventListener("click", () => { if ($ocrFile) $ocrFile.click(); });
    if ($ocrFile) $ocrFile.addEventListener("change", async () => {
        ...
        try {
            ...
            const res = await Tesseract.recognize(f, langs, {
                logger: (m) => {
                    if (m && m.status && m.progress != null && $status) {
    ...
    function renderChatsList() {
        if (!$chats) return; // Patch: avoid crash if #chats is missing
        $chats.innerHTML = "";
    ...
    async function health() {
        try {
            if (!$status) return; // Patch: avoid crash if #status is missing
